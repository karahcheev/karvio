...
This page covers audit event search.

All paths are relative to `/api/v1`. Requests require authentication. Project-scoped access is enforced by the backend.
...
- `sort_by`: `timestamp_utc`, `actor`, `action`, `resource`, `result`, `request_id`;
- `sort_order`: `asc`, `desc`.

```http
GET /api/v1/audit-logs?project_id=proj_1&resource_type=test_case&result=success&page=1&page_size=50
```

Responses include event identity, timestamp, actor, action, resource, result, IP/user agent, request id, and before/after metadata when available.
